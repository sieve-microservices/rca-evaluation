...
OPENRC = '/etc/rpc-openstack/openrc-maas'
TOKEN_FILE = '~/workbench/sandbox/rpc-openstack/etc/.auth_ref.json'
...
#    from ceilometerclient import client as ceilometer_client
    from ceilometerclient.client import exceptions as ceilometer_exc
except ImportError:
    def get_ceilometer_client(*args, **kwargs):
        status_err('Cannot import ceilometerclient')
else:
    def get_ceilometer_client(auth_token=None, bypass_url=None, previous_tries=0):
        if previous_tries > 3:
            return None
        ...
        if not auth_token:
            auth_token = keystone.auth_token
        if not bypass_url:
            bypass_url = get_endpoint_url_for_service('metering',
                                                      auth_ref,
                                                      get_endpoint_type(
                                                          auth_details))

        ceilometer_client = ceilometerclient.client.get_client('2',
                                 os_username=auth_details['OS_USERNAME'],
                                 os_password=auth_details['OS_PASSWORD'],
                                 os_tenant_name=auth_details['OS_TENANT_NAME'],
                                 os_auth_url=auth_details['OS_AUTH_URL'])

#        ceilometer = ceilometer_client.Client('2', auth_token=auth_token,
#                                  bypass_url=bypass_url,
#                                  insecure=auth_details['OS_API_INSECURE'])
        ...
        try:
            ...
            # Exceptions are only thrown when we try and do something
            [meter.user_id for meter in meters]

        except (ceilometer_exc.Unauthorized, ceilometer_exc.AuthorizationFailure,
                AttributeError) as e:
            # NOTE(mancdaz)ceilometer doesn't properly pass back unauth errors, but
            # in fact tries to re-auth, all by itself. But we didn't pass it
            # an auth_url, so it bombs out horribly with an error.
            ...
            ceilometer = get_ceilometer_client(auth_token, bypass_url, previous_tries + 1)

            # we only want to pass ClientException back to the calling poller
            # since this encapsulates all of our actual API failures. Other
            # exceptions will be treated as script/environmental issues and
            # sent to status_err
        except ceilometer_exc.ClientException:
            raise
        except Exception as e:
            status_err(str(e))
...
try:
    ...
    from keystoneclient.v2_0.client import exceptions as k2_exc
    from keystoneclient.v3 import client as k3_client
    from keystoneclient.v3.client import exceptions as k3_exc
except ImportError:
...
def get_auth_details(openrc_file=OPENRC):
    ...
    for key in auth_details.keys():
        if (key == "OS_API_INSECURE") or (key == "OS_AUTH_VERSION") or (key == "OS_PROJECT_DOMAIN_NAME") or (key == "OS_USER_DOMAIN_NAME") or (key == "OS_ENDPOINT_TYPE"):
            continue
...
def metric_json(name, metric_type, value, unit=None):
    ...
    # following the example of dummy.py, we start building the json output 
    # out of a OrderedDict()
    metric_json_dict = collections.OrderedDict(key=name, value=value, unit=unit)

    METRICS_JSON.append(metric_json_dict)

def metric_influx(measurement_name, metric_values):
    ...
    # surround strings with "...", otherwise telegraf complains of invalid 
    # booleans
    metric_influx_line = str(measurement_name) + " "

    for metric_name, value in metric_values.iteritems():
        metric_influx_line += metric_name + "=\"" + str(value) + "\","

    METRICS_INFLUX.append(metric_influx_line.rstrip(","))
...
#        # following the example of dummy.py, we simply print the json.dumps() 
#        # of a list of OrderedDict() objects
#        for _metric_json in METRICS_JSON:
#            print(json.dumps(_metric_json))

        # after a bit of discussion in the influxdb threads, i've decided to 
        # go with influxdb's own format instead of json, which is not very 
        # well supported by telegraf's input.exec module
        for _metric_influx in METRICS_INFLUX:
            print(_metric_influx)
